...
class Category(models.Model):
    ...
    meta_description = models.CharField(
        ...
        help_text = 'meta描述，有得天SEO，用逗号分隔')
    created_at = models.DateTimeField("创建时间",auto_now_add=True)
    updated_at = models.DateTimeField("更新时间",auto_now=True)
    # 子类
    class Meta:
        db_table = 'categories' #
        ordering = ['-created_at'] #按降序排序
        verbose_name_plural = 'Categories' #模型复数名称,默认和类名相同
    # print打印时显示的内容
    def __str__(self):
        return self.name
    # 可为模型实例创建URL
...
class Product(models.Model):
    ...
    created_at = models.DateTimeField("创建时间", auto_now_add=True)
    updated_at = models.DateTimeField("更新时间", auto_now=True)
    categories = models.ManyToManyField(Category) # 产品可属于多个分类
